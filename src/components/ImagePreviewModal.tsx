...
export function ImagePreviewModal({
  ...
  const handleImageUploaded = (imageUrl: string) => {
    // Navigate to the image editor with the seed image and uploaded overlay
    // This will open the CompleteImageEditor where users can position and adjust the product image
    window.open(
      `/campaigns/${campaignId}?seedImage=${encodeURIComponent(
        premiumImage?.image_url || draftImage?.image_url || ""
      )}&overlayImage=${encodeURIComponent(imageUrl)}`,
      "_blank"
    );
    setShowImageUpload(false);
    toast.success("Product image uploaded! Opening editor...");
  };
