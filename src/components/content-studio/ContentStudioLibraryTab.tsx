          ...
          {filteredItems.map((item, index) => (
            <div key={`${item.type}-${item.data.id}`}>
              {item.type === "text" ? (
                ...
                <div className="card rounded-lg p-4 hover:shadow-lg transition">
                  <div className="flex items-start space-x-4">
                    <div className="relative w-64 h-36 bg-gray-100 dark:bg-gray-800 rounded-lg overflow-hidden flex-shrink-0 group">
                      {item.data.thumbnail_url ? (
                        <>
                          ...
                          <div className="absolute bottom-2 right-2 bg-black bg-opacity-75 text-white text-xs px-2 py-1 rounded">
                            {item.data.actual_duration || item.data.requested_duration || 0}s
                          </div>
                        </>
                      ) : (
                        <div className="w-full h-full flex items-center justify-center">
