...
export function VideoEditorModal({
  ...
  const [maskCanvasRef, setMaskCanvasRef] = useState<HTMLCanvasElement | null>(null);
  ...
  const handleEraseWithAI = async () => {
    if (!previewVideoRef.current || !maskRef.current) {
      toast.error("Video or mask not ready");
      return;
    }
    ...
    // Convert to base64
    const imageBase64 = videoCanvas.toDataURL('image/jpeg', 0.9);
    ...
    try {
      ...
      // Replace the video frame with the inpainted result
      if (data.inpainted_image_url) {
        toast.success("Successfully removed text/objects!");

        // Update video source (in a real implementation, you'd want to replace the video entirely)
        // For now, we'll just show a success message
        // The actual video replacement would need backend support
            ...
            >
              <video
                ...
                onLoadedMetadata={(e) => {
                  ...
                  if (maskRef.current) {
                    maskRef.current.width = video.videoWidth;
                    maskRef.current.height = video.videoHeight;
                  }
                }}
              />
