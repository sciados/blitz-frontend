...
export default function CampaignsPage() {
  ...
  const fetchActiveLimit = async () => {
    try {
      const response = await api.get("/api/campaigns/active-limit");
  ...
  const handleStatusToggle = async (campaignId: number, newStatus: string) => {
    try {
      ...
      if (err.response?.status === 400) {
        ...
        if (errorData.code === "ACTIVE_CAMPAIGN_LIMIT_EXCEEDED") {
          alert(errorData.message);
        } else {
          alert(errorData.message || "Failed to update campaign status");
        }
      } else {
